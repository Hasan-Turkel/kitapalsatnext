...
const FindBook = () => {
  ...
  return (
    <section className="max-w-[840px] m-auto mt-10">
      <div className="my-5">
        <button className="rounded-lg border p-2" onClick={()=>setOpen(!open)}> Arama ekranını {open?'kapat':'aç'}</button>
      </div>
      <div className={"overflow-hidden transition-max-height duration-500 ease-in-out " + (open?'max-h-[1000px]': 'max-h-0')}>
        ...
        <div className="my-4">
          <button className=" text-white rounded-lg bg-blue-500 p-3 h-[40px] w-[350px] hover:bg-blue-600 transition-colors duration-500 ease-in-out">
            Ara
          </button>
        </div>
      </div>
